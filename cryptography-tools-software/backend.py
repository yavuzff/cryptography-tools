...
import importlib.util                       #used to import a file using a string name
...
def validateAffine(a,b):
    if validateCaesar(a) and validateCaesar(a):
    #the inputs must both be integers, reusing caesar code as same validation
        return True
    else:
        return False
...
def morse(encrypt,text):
    ...
    if encrypt: #dictionary if encrypting
        ...
        for word in text: #for each word in the text
            ...
            for i in letters:
                if i in '{#*^&_}': #some valid characters that are not in morse code
                    new += i
                elif i not in mappings: #checks whether the code is valid
...
def validateHill(a,b,c,d,alphabetLength): #checks if a matrix is valid and invertible modulo n
    ...
    for i in nums:
        if determinant % i == 0:
            return 'Invalid Matrix: Determinant of the matrix and alphabet length must be co-prime, try 2 3 1 2'
...
#Cipher Plug-in
def cipherPlugin(filepath,encrypt,key,text): #imports an external and encrypts using its filepath
    spec = importlib.util.spec_from_file_location("main",filepath) #the file is imported
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    
    #hasattr checks whether the function exists in the imported file
    if encrypt and hasattr(module,'encrypt'):
        #try used to see if the function has key,text as parameters
        try: output = module.encrypt(key,text) #the encrypt function in the file is called
        except: return False
        
    elif not encrypt and hasattr(module,'decrypt'):
        #try used to see if the function has key,text as parameters
        try: output = module.decrypt(key,text) #the encrypt function in the file is called
        except: return False
